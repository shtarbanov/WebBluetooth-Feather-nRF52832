...
void setupServicesAndCharacteristics(){
  ...
  batteryLevelCharacteristic = BLECharacteristic(0x2A19);
  batteryLevelCharacteristic.setProperties(CHR_PROPS_READ | CHR_PROPS_NOTIFY); 
  batteryLevelCharacteristic.setFixedLen(1);
  batteryLevelCharacteristic.setPermission(SECMODE_OPEN, SECMODE_NO_ACCESS);
...
void setup(){ 
  ...
  setupServicesAndCharacteristics(); // Start Battery Service
...
void loop(){ 
  if(millis() - lastTime > 5000){ //we wiil check the battery only once per 5 seconds. This reduces power consumption greatly.
    ...
    if(batteryPercentage < batteryPercentageOld-1 || batteryPercentage > batteryPercentageOld+1){
      batteryLevelCharacteristic.notify8(batteryPercentage);
...
void connect_callback(uint16_t conn_handle){ // callback invoked when central connects
  ...
  batteryLevelCharacteristic.notify8(batteryPercentageOld);
}
