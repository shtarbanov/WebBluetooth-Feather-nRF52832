 ...
 *      If the value is 0 - don't send a pressure.
 *      If the value is 1 - send a single pressure reading and set value back to 0.
 *      If the value is 2 - send a pressure notifications continuously at some interval.
 *
 *Every 3 seconds, a new pressure reading is made and a notification is sent with the value.
...
void onWrite(uint16_t conn_hdl, BLECharacteristic* chr, uint8_t* data, uint16_t len){
  ...
  if(len==1){
    ...
    if(data[0] == 0x01){
      ...
      notifyContinuously = true;
    }
    else if(data[0] == 0x00){
      notifyContinuously = false;
    }
  }
  //THe current value of the the characteristic does not matter, because we only check for the
  //value when it changes, but we never actually read this characteristic in our main loop.
  //Thus, even if we write 0x01, the notifyContinously state will remain whatever it was.
}
...
void loop(){ 
  if(millis() - lastTime > 3000 && notifyContinuously){ //we wiil check the battery only once per 5 seconds. This reduces power consumption greatly.
    notifyNewPressure();
    lastTime=millis();
  } 
